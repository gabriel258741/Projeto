...
case $arq2 in
	...
	7) p2=7; pero;;
	8) menu;;
	*) dialog --msgbox "Opção inválida" 0 0; bash /Projeto/config/garq.sh;;
esac
}
function pero(){
arq2=$( dialog						\
...
case $arq2 in
	...
	7) p3=7;;
	8) perg;;
	*) dialog --msgbox "Opção invalida" 0 0; bash /Projeto/config/garq.sh;;
esac
chmod $p1$p2$p3 $arq
case $? in
	0) dialog --msgbox "Permissões alteradas com sucesso" 0 0; bash /Projeto/config/garq.sh;;
...
case $? in
	...
	255) bash /Projeto/config/garq.sh;;
esac
arq2=$( dialog						\
	--stdout					\
	--title "Alterar permissão"			\
